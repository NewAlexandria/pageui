...
*/
	...
	$sql = "SELECT id FROM Users WHERE LCASE(username) LIKE '".strtolower($username)."'";
	...
	if (mysql_affected_rows()>0) {
		...
		header("Location: ../signup.php?username=".$username."&email=".$email."&fail=Username	+already+in+database");	
...
//	AddPages($user_id, 'default', 'intro.php');
	...
	$sql = "INSERT INTO GroupsPages (group_id, page_id, user_id, row, col, date_created) SELECT GP.group_id, GP.page_id, ".$user_id.", GP.row, GP.col, '".date("Y-m-d H:m:s")."' FROM GroupsPages GP INNER JOIN Users U ON GP.user_id = U.id  INNER JOIN Pages P ON GP.page_id = P.id WHERE U.username LIKE 'tutorial' AND P.Name = 'my first page'";
	$result = mysql_query ( $sql );
	$errtxt .= '<br />'.$sql;

	// get all the links and add them, too
	$sql = "INSERT INTO Links (URL, title, date_add, group_id, user_id, col, row, page_id) SELECT L.URL, L.title, '".date("Y-m-d H:m:s")."', L.group_id, ".$user_id.", L.col, L.row, L.page_id FROM Links L INNER JOIN Users U ON L.user_id = U.id WHERE U.username LIKE 'tutorial' AND L.group_id IN (SELECT group_id FROM GroupsPages GP INNER JOIN Pages P ON GP.page_id = P.id INNER JOIN Users U ON GP.user_id = U.id WHERE U.username LIKE 'tutorial' AND P.Name LIKE 'my first page')";
	$result = mysql_query ( $sql );
	$errtxt .= '<br />'.$sql;
