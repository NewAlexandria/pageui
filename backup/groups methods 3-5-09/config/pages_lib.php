...
function AddPages($user_id, $name, $page) {
	global $errtxt;
	/// check for DB attacks, and log
	
	/// validate email, variable presence

	$page_id = InsertPageIf( $name, $user_id, $page );
	...
	setcookie("page_id", $page_id, time()+60*60*24*$iDaystoExpire, '/');
}

function old_AddPages($user_id, $page_id, $name) {
	global $errtxt;
	/// check for DB attacks, and log
	...
	$errtxt .= '<br/> page_id was: '.$page_id; 
	$_SESSION['page_id'] = $page_id;
	$_COOKIE['page_id'] = $_SESSION['page_id'];
}


function UpdatePage($page_id, $user_id, $name, $page) {
	global $errtxt;
	...
	// update Links table
	$sql = "UPDATE Links SET page_id = ".$page_id." WHERE user_id = ".$user_id." and page_id = ".$orig_page_id."";
	$errtxt .= $sql.'<br/>'; 
	$result = mysql_query($sql);
	
	// update GP table
	$sql = "UPDATE GroupsPages SET page_id = ".$page_id." WHERE user_id = ".$user_id." and page_id = ".$orig_page_id."";
	$errtxt .= $sql.'<br/>'; 
	$result = mysql_query($sql);
	
	// update UP table
	$sql = "UPDATE UsersPages SET page_id = ".$page_id." WHERE user_id = ".$user_id." and page_id = ".$orig_page_id."";
	$errtxt .= $sql.'<br/>'; 
	$result = mysql_query($sql);	
...
function DeletePage( $page_id, $user_id ) {
	global $errtxt;
	
	// confirm the page exists, also gets sort for re-sorting of UP
	$sql = "SELECT * FROM UsersPages WHERE user_id = ".ReturnSecureString($user_id)." and page_id = ".$page_id."";
	$errtxt .= $sql.'<br/>'; 
	$result = mysql_query($sql);
	...
	if ( mysql_num_rows ( $result ) > 0 ) {
		...
		$sql = "DELETE FROM UsersPages WHERE user_id = ".ReturnSecureString($user_id)." and page_id = ".$page_id."";
		$errtxt .= $sql.'<br/>'; 
		$result = mysql_query($sql);
		
		// resort as needed
			$sql = "UPDATE UsersPages SET sort = sort-1 WHERE user_id = ".ReturnSecureString($user_id)." and page_id = ".$page_id." AND sort > ".$sort;
			$errtxt .= $sql.'<br/>'; 
			$result = mysql_query($sql);

		
		// remove the GP records
		$sql = "DELETE FROM GroupsPages WHERE user_id = ".ReturnSecureString($user_id)." and page_id = ".$page_id."";
		$errtxt .= $sql.'<br/>'; 
		$result = mysql_query($sql);
	
		// perform LinkHistory inserts
		$sql = "SELECT * FROM Links WHERE user_id = ".ReturnSecureString($user_id)." and page_id = ".$page_id."";
		$errtxt .= $sql.'<br/>'; 
		$result = mysql_query($sql);
		...
		// update GP table
		$sql = "DELETE FROM Links WHERE user_id = ".ReturnSecureString($user_id)." and page_id = ".$page_id."";
		$errtxt .= '<br/>'.$sql;
		$result = mysql_query($sql);
	...
	// set the next page
	$sql = "SELECT * FROM UsersPages WHERE user_id = ".ReturnSecureString($user_id)." ";
	$errtxt .= '<br/>'.$sql;
	$result = mysql_query($sql);
...
function InsertPageIf( $name, $user_id, $page ) {
	global $errtxt;
...
//	$sql = "SELECT * FROM Pages P LEFT OUTER JOIN UsersPages UP ON P.id = UP.page_id WHERE P.Name LIKE '".$page_name."' AND ( UP.user_id = ".$user_id." OR UP.user_id IS NULL )";
	$sql = "SELECT * FROM Pages P INNER JOIN UsersPages UP ON P.id = UP.page_id WHERE P.Name LIKE '".$page_name."' AND UP.user_id = ".$user_id."";
	...
	// thus, do they already have the page?
	if (mysql_num_rows($result)>0) {
		// mos of this is commented out now, but let here in case there is an assured SQL-way to check for the user page given the commented out SQL above
//		$r = mysql_fetch_assoc($result);
//		$page_id = $r['id'];
...
//		}  // ^ ELSE  the page exists, but they don't have a UP record yeti
	...
	$errtxt .= 'insertPageIF return: '.$page_id;
	return $page_id;
}
