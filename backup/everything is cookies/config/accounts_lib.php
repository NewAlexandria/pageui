...
function Link_GetNewRowCol ( $group_id ) {
		...
		$sql = "SELECT L.row as row, L.col as col FROM GroupsPages GP INNER JOIN Links L ON GP.group_id = L.group_id WHERE L.user_id = GP.user_id AND L.page_id = GP.page_id AND GP.group_id = ".$group_id." AND L.user_id = ".$_SESSION['user_id']." AND GP.page_id = ".$_COOKIE['page_id']." ORDER BY L.col, L.row ASC";
		$errtxt .= '<BR/> '.$sql;
